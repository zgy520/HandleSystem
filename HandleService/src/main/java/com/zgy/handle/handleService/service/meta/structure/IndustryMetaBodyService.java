...
public class IndustryMetaBodyService extends SystemService<IndustryMetaBody, IndustryMetaBodyDTO> {
    ...
    public void createRegisterMetaDataXml(IndustryMetaHeader metaHeader, List<IndustryMetaBody> metaBodyList){
        EnterprisePre enterprisePre = entepriseFeignClient.getEnterpriseInfo(getDepartId());
        ...
        try {
            ...
            for (IndustryMetaBody metaBody : metaBodyList){
            ...
            if (metaHeader.getParent() != null){
                xmlHeader = new XmlHeader(metaHeader.getHeader().getIdentityNum(),getMetaNodeLevel(metaHeader),metaHeader.getParent().getHeader().getIdentityNum());
            }else {
                xmlHeader = new XmlHeader(metaHeader.getHeader().getIdentityNum(),getMetaNodeLevel(metaHeader),null);
            }
    ...
    private String getRootIdentityNum(IndustryMetaHeader metaHeader){
        String rootIdentity =  "";
        if (metaHeader.getParent() == null){
            rootIdentity = metaHeader.getHeader().getIdentityNum();
        }else {
    ...
    private int getMetaNodeLevel(IndustryMetaHeader metaHeader){
        int level = 0;
        if(metaHeader.getParent()==null){
            level = 1;
        }
